...
Each deck lives in its own subdirectory; outputs land next to the source:

```
presentations/foo/foo.dot   →   presentations/foo/foo.svg  +  presentations/foo/foo.html
```

The generated HTML loads `runtime.js` and `runtime.css` from the dot-slides
root via a relative path (`../../runtime.js`, `../../runtime.css` for a deck
one subdirectory deep — the build computes the depth), so it works both from
a local HTTP server (Hugo) and from `file://` directly. Per-deck assets like
images go in `presentations/foo/img/` and are referenced as `img/…`.
...
The HTML inlines the SVG and references the shared runtime via a relative
path back to the dot-slides root (`../../runtime.js`, `../../runtime.css`
for a deck one subdirectory deep — the build computes the depth and fills
`{{RUNTIME_PREFIX}}`). Keep each deck in `presentations/<name>/`; relocating
a deck to a different depth is fine because the prefix is computed, but
moving the runtime files would break the references.
...
Build it (`python3 build_presentation.py presentations/example/example.dot`)
and open `presentations/example/example.html`. Arrow keys move between slides.
...
| `presentations/<name>/<name>.dot`  | Sources (hand-authored — use `class="seq"` on narrative edges)       |
| `presentations/<name>/<name>.svg`  | Generated by `dot -Tsvg` (tracked, so diffs catch layout drift)      |
| `presentations/<name>/<name>.html` | Generated; tiny chrome + inlined SVG + shared runtime references     |
| `presentations/<name>/img/`        | Optional per-deck image assets, referenced as `img/…`                |
